...
class NPNMultiRunner(object):
    ...
    def fit(self, model, data_loader, device, epoch):
        ...
        for j, data in tqdm(enumerate(data_loader), ncols=100, mininterval=1):
            ...
            target = torch.zeros([len(target_set), 8])
            for i in range(len(target_set)):
                target[i, int(target_set.data[i])] = 1
            y_pred, zero_pre, one_pre, two_pre, three_pre, four_pre, five_pre, six_pre, seven_pre = model(imgs)
            ...
            loss_four = self.criterion(four_pre, target[:, 4].to(device))
            loss_five = self.criterion(five_pre, target[:, 5].to(device))
            loss_six = self.criterion(six_pre, target[:, 6].to(device))
            loss_seven = self.criterion(seven_pre, target[:, 7].to(device))
            loss = loss_zero + loss_one + loss_two + loss_three + loss_four + loss_five + loss_six + loss_seven
