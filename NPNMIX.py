...
import numpy as np
from mlpMixer import ProjectionMixer


class NPNMIX(nn.Module):
    def __init__(self, device, class_num, batch_size):
        super(NPNMIX, self).__init__()
        self.device = device
        ...
        self.belong_to = ProjectionMixer(1, self.obj_dimension)
    ...
    def predict(self, x):
        ...
        d3 = d2.reshape(self.class_num*d.shape[0], self.obj_dimension)
        concepts2 = concepts1.reshape(self.class_num*d.shape[0], self.obj_dimension)
        #concepts2 = torch.cat((d2, concepts1), dim=-1)
        concepts3 = self.belong_to(d3, concepts2)
        concepts3 = concepts3.reshape(d.shape[0], self.class_num, self.obj_dimension)
